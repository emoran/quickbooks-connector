...
import com.intuit.ipp.data.holders.PaymentExpressionHolder;
...
public class PaymentExpressionHolderTransformer
    ...
    implements DiscoverableTransformer, MessageTransformer
{
    ...
    public boolean isSourceTypeSupported(Class<?> aClass) {
        return (aClass == PaymentExpressionHolder.class);
    }

    public boolean isSourceDataTypeSupported(DataType<?> dataType) {
        return (dataType.getType() == PaymentExpressionHolder.class);
    }

    public List<Class<?>> getSourceTypes() {
        return Arrays.asList(new Class<?> [] {PaymentExpressionHolder.class });
    }

    public List<DataType<?>> getSourceDataTypes() {
        return Arrays.asList(new DataType<?> [] {DataTypeFactory.create(PaymentExpressionHolder.class)});
    }
    ...
    public DataType<?> getReturnDataType() {
        return DataTypeFactory.create(Payment.class);
    }
        ...
        throws TransformerMessagingException
    {
        ...
        PaymentExpressionHolder holder = ((PaymentExpressionHolder) src);
        Payment result = new Payment();
        try {
            final ReferenceType _transformedCustomerRef = ((ReferenceType) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_customerRefType").getGenericType(), null, holder.getCustomerRef()));
            result.setCustomerRef(_transformedCustomerRef);
            final ReferenceType _transformedRemitToRef = ((ReferenceType) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_remitToRefType").getGenericType(), null, holder.getRemitToRef()));
            result.setRemitToRef(_transformedRemitToRef);
            final ReferenceType _transformedDepositToAccountRef = ((ReferenceType) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_depositToAccountRefType").getGenericType(), null, holder.getDepositToAccountRef()));
            result.setDepositToAccountRef(_transformedDepositToAccountRef);
            final ReferenceType _transformedPaymentMethodRef = ((ReferenceType) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_paymentMethodRefType").getGenericType(), null, holder.getPaymentMethodRef()));
            result.setPaymentMethodRef(_transformedPaymentMethodRef);
            final String _transformedPaymentRefNum = ((String) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_paymentRefNumType").getGenericType(), null, holder.getPaymentRefNum()));
            result.setPaymentRefNum(_transformedPaymentRefNum);
            final PaymentTypeEnum _transformedPaymentType = ((PaymentTypeEnum) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_paymentTypeType").getGenericType(), null, holder.getPaymentType()));
            result.setPaymentType(_transformedPaymentType);
            final CheckPayment _transformedCheckPayment = ((CheckPayment) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_checkPaymentType").getGenericType(), null, holder.getCheckPayment()));
            result.setCheckPayment(_transformedCheckPayment);
            final CreditCardPayment _transformedCreditCardPayment = ((CreditCardPayment) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_creditCardPaymentType").getGenericType(), null, holder.getCreditCardPayment()));
            result.setCreditCardPayment(_transformedCreditCardPayment);
            final BigDecimal _transformedTotalAmt = ((BigDecimal) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_totalAmtType").getGenericType(), null, holder.getTotalAmt()));
            result.setTotalAmt(_transformedTotalAmt);
            final BigDecimal _transformedUnappliedAmt = ((BigDecimal) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_unappliedAmtType").getGenericType(), null, holder.getUnappliedAmt()));
            result.setUnappliedAmt(_transformedUnappliedAmt);
            final Boolean _transformedProcessPayment = ((Boolean) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_processPaymentType").getGenericType(), null, holder.getProcessPayment()));
            result.setProcessPayment(_transformedProcessPayment);
            final IntuitAnyType _transformedPaymentEx = ((IntuitAnyType) evaluateAndTransform(this.muleContext, event, PaymentExpressionHolder.class.getDeclaredField("_paymentExType").getGenericType(), null, holder.getPaymentEx()));
            result.setPaymentEx(_transformedPaymentEx);
