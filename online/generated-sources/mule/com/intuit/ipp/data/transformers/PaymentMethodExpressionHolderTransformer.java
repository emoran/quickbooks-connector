...
public class PaymentMethodExpressionHolderTransformer
    ...
    implements DiscoverableTransformer, MessageTransformer
{
    ...
    public boolean isSourceTypeSupported(Class<?> aClass) {
        return (aClass == PaymentMethodExpressionHolder.class);
    }

    public boolean isSourceDataTypeSupported(DataType<?> dataType) {
        return (dataType.getType() == PaymentMethodExpressionHolder.class);
    }

    public List<Class<?>> getSourceTypes() {
        return Arrays.asList(new Class<?> [] {PaymentMethodExpressionHolder.class });
    }

    public List<DataType<?>> getSourceDataTypes() {
        return Arrays.asList(new DataType<?> [] {DataTypeFactory.create(PaymentMethodExpressionHolder.class)});
    }
    ...
    public DataType<?> getReturnDataType() {
        return DataTypeFactory.create(PaymentMethod.class);
    }
        ...
        throws TransformerMessagingException
    {
        ...
        PaymentMethodExpressionHolder holder = ((PaymentMethodExpressionHolder) src);
        PaymentMethod result = new PaymentMethod();
        try {
            final String _transformedName = ((String) evaluateAndTransform(this.muleContext, event, PaymentMethodExpressionHolder.class.getDeclaredField("_nameType").getGenericType(), null, holder.getName()));
            result.setName(_transformedName);
            final Boolean _transformedActive = ((Boolean) evaluateAndTransform(this.muleContext, event, PaymentMethodExpressionHolder.class.getDeclaredField("_activeType").getGenericType(), null, holder.getActive()));
            result.setActive(_transformedActive);
            final String _transformedType = ((String) evaluateAndTransform(this.muleContext, event, PaymentMethodExpressionHolder.class.getDeclaredField("_typeType").getGenericType(), null, holder.getType()));
            result.setType(_transformedType);
            final IntuitAnyType _transformedPaymentMethodEx = ((IntuitAnyType) evaluateAndTransform(this.muleContext, event, PaymentMethodExpressionHolder.class.getDeclaredField("_paymentMethodExType").getGenericType(), null, holder.getPaymentMethodEx()));
            result.setPaymentMethodEx(_transformedPaymentMethodEx);
