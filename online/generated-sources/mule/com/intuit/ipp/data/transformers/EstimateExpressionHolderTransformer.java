...
public class EstimateExpressionHolderTransformer
    ...
    implements DiscoverableTransformer, MessageTransformer
{
    ...
    public boolean isSourceTypeSupported(Class<?> aClass) {
        return (aClass == EstimateExpressionHolder.class);
    }
    ...
    public List<Class<?>> getSourceTypes() {
        return Arrays.asList(new Class<?> [] {EstimateExpressionHolder.class });
    }

    public List<DataType<?>> getSourceDataTypes() {
        return Arrays.asList(new DataType<?> [] {DataTypeFactory.create(EstimateExpressionHolder.class)});
    }
        ...
        throws TransformerMessagingException
    {
        ...
        EstimateExpressionHolder holder = ((EstimateExpressionHolder) src);
        Estimate result = new Estimate();
        try {
            final Date _transformedExpirationDate = ((Date) evaluateAndTransform(this.muleContext, event, EstimateExpressionHolder.class.getDeclaredField("_expirationDateType").getGenericType(), null, holder.getExpirationDate()));
            result.setExpirationDate(_transformedExpirationDate);
            final String _transformedAcceptedBy = ((String) evaluateAndTransform(this.muleContext, event, EstimateExpressionHolder.class.getDeclaredField("_acceptedByType").getGenericType(), null, holder.getAcceptedBy()));
            result.setAcceptedBy(_transformedAcceptedBy);
            final Date _transformedAcceptedDate = ((Date) evaluateAndTransform(this.muleContext, event, EstimateExpressionHolder.class.getDeclaredField("_acceptedDateType").getGenericType(), null, holder.getAcceptedDate()));
            result.setAcceptedDate(_transformedAcceptedDate);
            final IntuitAnyType _transformedEstimateEx = ((IntuitAnyType) evaluateAndTransform(this.muleContext, event, EstimateExpressionHolder.class.getDeclaredField("_estimateExType").getGenericType(), null, holder.getEstimateEx()));
            result.setEstimateEx(_transformedEstimateEx);
