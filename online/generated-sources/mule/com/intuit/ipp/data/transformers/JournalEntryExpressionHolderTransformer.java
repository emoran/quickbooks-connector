...
import com.intuit.ipp.data.holders.JournalEntryExpressionHolder;
...
public class JournalEntryExpressionHolderTransformer
    ...
    implements DiscoverableTransformer, MessageTransformer
{
    ...
    public boolean isSourceTypeSupported(Class<?> aClass) {
        return (aClass == JournalEntryExpressionHolder.class);
    }

    public boolean isSourceDataTypeSupported(DataType<?> dataType) {
        return (dataType.getType() == JournalEntryExpressionHolder.class);
    }

    public List<Class<?>> getSourceTypes() {
        return Arrays.asList(new Class<?> [] {JournalEntryExpressionHolder.class });
    }

    public List<DataType<?>> getSourceDataTypes() {
        return Arrays.asList(new DataType<?> [] {DataTypeFactory.create(JournalEntryExpressionHolder.class)});
    }
    ...
    public DataType<?> getReturnDataType() {
        return DataTypeFactory.create(JournalEntry.class);
    }
        ...
        throws TransformerMessagingException
    {
        ...
        JournalEntryExpressionHolder holder = ((JournalEntryExpressionHolder) src);
        JournalEntry result = new JournalEntry();
        try {
            final Boolean _transformedAdjustment = ((Boolean) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_adjustmentType").getGenericType(), null, holder.getAdjustment()));
            result.setAdjustment(_transformedAdjustment);
            final Boolean _transformedHomeCurrencyAdjustment = ((Boolean) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_homeCurrencyAdjustmentType").getGenericType(), null, holder.getHomeCurrencyAdjustment()));
            result.setHomeCurrencyAdjustment(_transformedHomeCurrencyAdjustment);
            final Boolean _transformedEnteredInHomeCurrency = ((Boolean) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_enteredInHomeCurrencyType").getGenericType(), null, holder.getEnteredInHomeCurrency()));
            result.setEnteredInHomeCurrency(_transformedEnteredInHomeCurrency);
            final BigDecimal _transformedTotalAmt = ((BigDecimal) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_totalAmtType").getGenericType(), null, holder.getTotalAmt()));
            result.setTotalAmt(_transformedTotalAmt);
            final BigDecimal _transformedHomeTotalAmt = ((BigDecimal) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_homeTotalAmtType").getGenericType(), null, holder.getHomeTotalAmt()));
            result.setHomeTotalAmt(_transformedHomeTotalAmt);
            final IntuitAnyType _transformedJournalEntryEx = ((IntuitAnyType) evaluateAndTransform(this.muleContext, event, JournalEntryExpressionHolder.class.getDeclaredField("_journalEntryExType").getGenericType(), null, holder.getJournalEntryEx()));
            result.setJournalEntryEx(_transformedJournalEntryEx);
