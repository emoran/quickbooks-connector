 ...
 * 
 *         Product: QBW
 *         Description: PurchaseOrder is a non-posting transaction representing a request to purchase goods or services from a third party. Adds a purchase order, which is an order submitted to a vendor. You can use ItemReceipt Add to receive items against a purchase order as items arrive from the vendor. A purchase order is a non-posting transaction, so it does not affect a companys balance sheet or income statement.
